...
int Task8(const int arr[], const unsigned size) {
	...
	return sum;
	//Return the sum of the members with uneven indexes in the array, return 0 if there is only 1 member
}
...
bool Task10(const int arr[], const unsigned size) {
	...
	return false;
	//Return "True" if there are 2 equal adjacent members and "False" if there aren't
}
...
bool Task18(const char arr[]) {
	bool U = false, P = true;
	for (unsigned i = 0; i < Task16(arr) { && !(U && P); ++i) //if both U and P are true, then the loop will end
		if (arr[i] == 'U') {
			U = true;
		}
